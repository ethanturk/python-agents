...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_perform_rag_success(mocker, mock_openai_agent):
    ...
    # Let's mock db_service.search directly to test Agent logic in perform_rag
    ...
    # We need to patch a method on the instance
    mocker.patch.object(db_service, "search", return_value=mock_results)

    result = await perform_rag("question")
