...
@pytest.mark.unit
def test_search_documents(client, mock_vector_db, mock_openai_agent, mocker):
    ...
    mocker.patch("backend_app.perform_rag", return_value={"answer": "Mocked", "results": []})
...
@pytest.mark.unit
def test_search_qa_endpoint(client, mock_openai_agent, mocker):
    payload = {
    ...
    mocker.patch("backend_app.run_qa_agent", return_value="Mocked LLM response")
