...
Given notifications function receives a notification with summary data
When notification status is completed and a result is provided
Then system attempts to save to SQLite database
...
The notifications serverless function MUST be implemented in Node.js for unified TypeScript stack.

#### Scenario: Node.js poll endpoint works

Given: notifications function is implemented in Node.js
And notifications have been pushed to in-memory queue
When user GETs to `/poll` with a since_id parameter
Then system returns notifications since that ID
And waits up to 20 seconds for new notifications

#### Scenario: Node.js notify endpoint works

Given: notifications function is implemented in Node.js
And a task completes
When: system POSTs to `/internal/notify` with notification details
Then: system pushes notification to in-memory queue
And saves completed summaries to SQLite database
And returns success status

### Requirement: Node.js Notifications Dependencies

The notifications serverless function MUST use Node.js dependencies instead of Python packages.

#### Scenario: Node.js dependencies are minimal

Given: notifications function is built
When package.json is inspected
Then: it has minimal dependencies (no external AI/storage/DB libs)
And: it excludes Python packages (fastapi, etc.)

### Requirement: Notifications Function Testing

The notifications serverless function MUST have unit tests covering all endpoints.

#### Scenario: All notifications endpoints have tests

Given: notifications function is implemented
When test suite is run
Then all 13 tests pass
And coverage includes poll, notify, and queue operations
