...
import { getDocuments, getDocumentSets, deleteDocuments } from '../../backend-nodejs/common/supabase.js';
import { uploadFile, downloadFile, deleteFile } from '../../backend-nodejs/common/azure.js';
...
export default async function handler(request: Request, context: any) {
  logger.info({ method: request.method, url: request.url }, 'Documents request');
  ...
  try {
    ...
    if (pathname === '/agent/documentsets') {
      ...
      return Response.json({ document_sets: documentSets } as DocumentSetsResponse);
    }
    ...
    // DELETE document
    if (pathname.startsWith('/agent/documents/') && request.method === 'DELETE') {
    ...
    if (pathname.startsWith('/agent/files/')) {
      ...
      if (parts.length > 1) {
        ...
        const { buffer, contentType } = await downloadFile(filename, documentSet);
