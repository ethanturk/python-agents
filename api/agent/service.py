"""
Agent service wrapper for serverless deployment.

This module provides a thin wrapper around backend agent services,
using minimal dependencies (pydantic-ai, litellm, supabase).
"""
...
def perform_rag(query: str, limit: int = 10, document_set: str = None) -> dict:
    ...
    results = asyncio.run(db_service.search(query, limit, document_set))

    if not results:
        return {
            "answer": "I couldn't find any relevant information in knowledge base.",
            "results": [],
        }
