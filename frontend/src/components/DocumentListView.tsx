...
import { ChevronDown, Trash2, FileText, FileText as SummarizeIcon, Upload } from "lucide-react";
...
const DocumentRow = memo(({ filename, chunks, onSummarize, onDelete }: DocumentRowProps) => {
...
export default function DocumentListView({
  ...
  const docsBySet = useMemo(() => {
    const groups: Record<string, Array<{ filename: string; chunks: ChunkData[] }>> = {};
            ...
            return (
              ...
              >
                <AccordionItem value={set}>
                  <AccordionTrigger className={`px-3 ${isExpanded ? "bg-muted/50" : ""}`}>
                    <h3 className="font-semibold">{set} ({docs.length})</h3>
                  </AccordionTrigger>
                  <AccordionContent>
